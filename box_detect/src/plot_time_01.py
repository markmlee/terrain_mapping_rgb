...
with open('/home/rainbow/Desktop/com_compare.csv','r') as csvfile:
    ...
    for row in plots:
    	...
    	if(row_count >0 and row_count < 245):
...
print("indexSize: ",len(indexList), "y size:", len(y1), len(y2), len(y3))

plt.plot(indexList, y1, label='visual odom', marker='.', color='b')
plt.plot(indexList, y2, label='kinematic reference', marker='.', color='r')
plt.plot(indexList, y3, label='diff', marker='.', color='k')
