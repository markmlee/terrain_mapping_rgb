...
//yellow
int minH = 9, maxH = 49, minS = 172, maxS = 252, minV = 116, maxV = 201;
...
void cloud_cb (sensor_msgs::PointCloud2ConstPtr const& input)
{
   ...
   // ROS_INFO("====== State 1. Before PCloud CB. SizePixelArray: %lu\n ====== ", stepsArray_pixel.poses.size());
	...
	//ROS_INFO("====== State 1. After PCloud CB\n ====== ");
...
int main(int argc, char **argv)
{
    ...
    while(nh.ok())
    {
       ...
       // ROS_INFO("====== State 2 END. PUBLISHED TF FOR DETECTED STEP ARRAY  ====== ");
		...
		double step_planning_time = (endTime - beginTime).toSec() * 1000;
		//ROS_INFO("Time (ms) taken within while loop: %f\n",step_planning_time);
		
	
		//==========================================
		//create TF array filter
		if(stateMachine_counter == 4)
		{
			stateMachine_counter = 5;
			//ROS_INFO("====== State 5 Publish the Averaged TF Array ======= \n");
			...
			for(int i = 0; i < stepsArray_pose_mean.poses.size(); i ++ ) 
			{
				...
				br.sendTransform(tf::StampedTransform(tf_filter_steps, ros::Time::now(), "/world", step_name.str()));
				//ROS_INFO("step%d, X: %f, Y%f\n",i,stepsArray_pose_mean.poses[i].position.x,stepsArray_pose_mean.poses[i].position.y );

			}
			
			//ROS_INFO("====== State 5 END. PUBLISHED STEPARRAY MEAN ====== ");
		}
		 ...
		 if(stateMachine_counter == 5) {
			...
			//ROS_INFO("====== State 6 END. Emptied the Vector ====== ");
