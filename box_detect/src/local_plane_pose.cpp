...
//yellow
int minH = 15, maxH = 49, minS = 120, maxS = 255, minV = 140, maxV = 240;
...
#define contourAreaThreshold 1500
#define boxLikeRatioMax      1.75
...
geometry_msgs::PoseArray box_corner_array;
...
bool compare_by_sortY(geometry_msgs::Pose pose_a, geometry_msgs::Pose pose_b)
{
	return (pose_a.position.y > pose_b.position.y); //sort by ascending order
}
...
void getBoundingBox(std::vector<std::vector<cv::Point> > contourInput, cv::Mat imageIn)
{
    ...
    for (int contourIndex = 0; contourIndex  < contourInput.size(); contourIndex++)
    {
        ...
        if ((cv::contourArea(contourInput[contourIndex])) > contourAreaThreshold)
        {
			...
			//std::cout << "Found Box! counterarea: " << cv::contourArea(contourInput[contourIndex]) << std::endl;
            ...
            if( ((height / width) < boxLikeRatioMax) && ((height / width) > boxLikeRatioMin) )
            {
                ...
                // ========================== get angle of the box ==========================
                //update new pts

                for(int i = 0; i < 4; i++) 
                {
					geometry_msgs::Pose temp_pose;
					temp_pose.position.x = box_points[i].x;
					temp_pose.position.y = box_points[i].y;
					box_corner_array.poses.push_back(temp_pose);
				}
                
                //1. sort corners by Y
                for(int i = 0; i < box_corner_array.poses.size(); i ++) 
                {
					//ROS_INFO("before X: %f, Y:%f\n", box_corner_array.poses[i].position.x, box_corner_array.poses[i].position.y);
				}
				
                sort(box_corner_array.poses.begin(), box_corner_array.poses.end(), compare_by_sortY);
                
                for(int i = 0; i < box_corner_array.poses.size(); i ++) 
                {
					//ROS_INFO("after X: %f, Y:%f\n", box_corner_array.poses[i].position.x, box_corner_array.poses[i].position.y);
				}
                //2. get biggest 2 Y corner
                float deltaX = box_corner_array.poses[1].position.x - box_corner_array.poses[0].position.x; 
                float deltaY = box_corner_array.poses[1].position.y - box_corner_array.poses[0].position.y;
                //ROS_INFO("dX: %f, dY: %f\n",deltaX, deltaY);
                //3. get angle from 2 corners
                float step_angle = -atanf(deltaY/deltaX);
                float step_angle_deg = step_angle * R2D;
                
                
                while (!box_corner_array.poses.empty())
			  {
				box_corner_array.poses.pop_back();
			  }
			  
                //printf("i: %d, Box X,Y: (%f,%f), Angle: %f\n", contourIndex, box.center.x, box.center.y, step_angle_deg);
                ...
                myQuaternion.setRPY( 0, 0,  step_angle);
                
                step_pixel.orientation.z = (float)myQuaternion.z();
                step_pixel.orientation.w = (float)myQuaternion.w();
                
                stepsArray_pixel.poses.push_back(step_pixel);
                
                //ROS_INFO("quat x: %f, y:%f, z:%f, w:%f\n",step_pixel.orientation.x, step_pixel.orientation.y, step_pixel.orientation.z, step_pixel.orientation.w);
                //ROS_INFO("step degree: %f\n",step_angle_deg);
               
               /* FILTER BY ANGLE IF 0... CUT OFF 
                if(step_angle_deg != 0)
                {
					stepsArray_pixel.poses.push_back(step_pixel);
				}
				...
				{
					ROS_ERROR("found step that is too close");
					 ROS_INFO("pixelArea: %f, angle_deg: %f\n",cv::contourArea(contourInput[contourIndex]), step_angle_deg);
				}
				*/
    ...
    while(nh.ok())
    {
		...
		if(stateMachine_counter == 4)
		{
			...
			for(int i = 0; i <  stepsArray_filtered.size(); i++){
				...
				//add only if X value is legit. minimum FOV is 0.4m
				if(stepsArray_pose_mean.poses[i].position.x > 0.45)
				{
			...
			for(int i = 0; i <  stepsArray_pose_mean.poses.size(); i++){
				ROS_WARN("END MeanArray Index: %d, uX: %f, uY: %f,  uZ: %f, qz: %f, qw: %f\n",i,stepsArray_pose_mean.poses[i].position.x, stepsArray_pose_mean.poses[i].position.y, stepsArray_pose_mean.poses[i].position.z, stepsArray_pixel.poses[i].orientation.z, stepsArray_pixel.poses[i].orientation.w);
			...
			if(stateMachine_loopCompletion < 1)
			 ...
			 else 
			 {
				...
				for(int i = 0; i < stepsArray_pose_mean.poses.size(); i ++ ) 
				{
				  ...
				  //modify here for step orientation
					tf::Quaternion temp_Quaternion(stepsArray_pixel.poses[i].orientation.x, stepsArray_pixel.poses[i].orientation.y, stepsArray_pixel.poses[i].orientation.z, stepsArray_pixel.poses[i].orientation.w);	
					tf_filter_steps.setRotation(temp_Quaternion);
					//ROS_INFO("i:%d, qx: %f, y:%f, z:%f, w:%f\n",i, temp_Quaternion.x(), temp_Quaternion.y(),temp_Quaternion.z(),temp_Quaternion.w());
