...
void ModelDialog::DisplayUpdate(){
    if(ui->CB_USE_ENCODER->isChecked()){
        ...
        //gazelle
        //std::cout<<JointReference(4)<<", "<<JointReference(5)<<", "<<JointReference(10)<<", "<<JointReference(11)<<std::endl;
        double RAP_deg, RAR_deg;
        GK.FK_diff_Ankle_right(JointEncoder(4), JointEncoder(5),0,0,RAP_deg, RAR_deg);
        glWidget->model->ref[4+7] = RAP_deg * D2Rf;
        glWidget->model->ref[5+7] = RAR_deg * D2Rf;

        double LAP_deg, LAR_deg;
        GK.FK_diff_Ankle_left(JointEncoder(10), JointEncoder(11),0,0,LAP_deg, LAR_deg);
        glWidget->model->ref[10+7] = LAP_deg * D2Rf;
        glWidget->model->ref[11+7] = LAR_deg * D2Rf;
        ...
        //std::cout<<JointReference(4)<<", "<<JointReference(5)<<", "<<JointReference(10)<<", "<<JointReference(11)<<std::endl;
        double RAP_deg, RAR_deg;
        GK.FK_diff_Ankle_right(JointReference(4), JointReference(5),0,0,RAP_deg, RAR_deg);
        glWidget->model->ref[4+7] = RAP_deg * D2Rf;
        glWidget->model->ref[5+7] = RAR_deg * D2Rf;

        double LAP_deg, LAR_deg;
        GK.FK_diff_Ankle_left(JointReference(10), JointReference(11),0,0,LAP_deg, LAR_deg);
        glWidget->model->ref[10+7] = LAP_deg * D2Rf;
        glWidget->model->ref[11+7] = LAR_deg * D2Rf;
    }
...
HUBOModel* GLWidget::newModel(){
    ...
    *bones[B_LWFT] + tempbone[20] + joints[LF_c1] + bones[B_LF_c1] + tempbone[29] + joints[LF_c2] + bones[B_LF_c2] + tempbone[32] + joints[LF_c3] + bones[B_LF_c3];
    ...
    tempbone[7]  = new Bone("W_RH",   isnl::pos(0,-0.06,0));
    tempbone[35] = new Bone("RHR_RHP",   isnl::pos(0,-0.0491, -0.072));
    tempbone[8]  = new Bone("RHP_RKN", isnl::pos(0.,0,-0.33));
    tempbone[9]  = new Bone("RKN_RA", isnl::pos(0,0,-0.3225));
    tempbone[13] = new Bone("RA_RA2", isnl::pos(0,0,-0.0));
    *bones[B_WST] +tempbone[7]+ joints[RHY] + bones[B_RHY]+joints[RHR] + bones[B_RHR] + tempbone[35] + joints[RHP] + bones[B_RHP]
            +tempbone[8]+ joints[RKN] + bones[B_RKN]
            +tempbone[9]+joints[RAP] + bones[B_RAP]+ tempbone[13]+joints[RAR] + bones[B_RAR];

    tempbone[10] = new Bone("W_LH",   isnl::pos(0,0.06,0));
    tempbone[36] = new Bone("LHR_LHP", isnl::pos(0, 0.0491, -0.072));
    tempbone[11] = new Bone("LHP_LKN", isnl::pos(0,0,-0.33));
    tempbone[12] = new Bone("LKN_LA", isnl::pos(0,0,-0.3225));
    tempbone[14] = new Bone("LA_LA2", isnl::pos(0,0,-0.0));
    *bones[B_WST] + tempbone[10]+joints[LHY] + bones[B_LHY]+joints[LHR] + bones[B_LHR] + tempbone[36] + joints[LHP] + bones[B_LHP]
