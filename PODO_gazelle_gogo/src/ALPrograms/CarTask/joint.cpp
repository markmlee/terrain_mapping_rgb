...
int JointControlClass::RefreshToCurrentEncoder(){
    if(Shm_sen->ENCODER[MC_ID_CH_Pairs[RKN].id][MC_ID_CH_Pairs[RKN].ch].CurrentPosition < 0.0001 && Shm_sen->ENCODER[MC_ID_CH_Pairs[RKN].id][MC_ID_CH_Pairs[RKN].ch].CurrentPosition > -0.0001){
        ...
        mcId = Joints[WST]->GetId();
        mcCh = Joints[WST]->GetCh();

        Joints[WST]->SetRefAngleCurrent(Shm_sen->ENCODER[mcId][mcCh].CurrentPosition);
        Shm_ref->JointReference[PODONum][mcId][mcCh] = Joints[WST]->GetRefAngleCurrent();

    }
    return 1; // refresh done!
}

int JointControlClass::RefreshToCurrentEncoder_TEST(){
        ...
        std::cout<<Shm_sen->ENCODER[13][0].CurrentPosition<<std::endl;
        std::cout<<Shm_ref->JointReference[PODONum][13][0]<<std::endl;
