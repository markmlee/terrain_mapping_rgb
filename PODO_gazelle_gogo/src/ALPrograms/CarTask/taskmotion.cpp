...
TaskMotion::TaskMotion(pRBCORE_SHM_REFERENCE _shm_ref, pRBCORE_SHM_SENSOR _shm_sen, pRBCORE_SHM_COMMAND _shm_com, JointControlClass *_joint){
    ...
    kine_drc.upper_limit[idRSY] = 190*D2R;
    kine_drc.lower_limit[idRSY] = -100*D2R;
    kine_drc.upper_limit[idLSY] = 100*D2R;
    kine_drc.lower_limit[idLSY] = -190*D2R;
...
void TaskMotion::RefreshToCurrentReferencePELV_LH(void)
{
    kine_drc.FK_RightFoot_Local(Q_filt_34x1, pRF_3x1, qRF_4x1);
    kine_drc.FK_LeftFoot_Local(Q_filt_34x1,  pLF_3x1, qLF_4x1);
    kine_drc.FK_RightHand_Local(Q_filt_34x1, LOCAL_UB, pRH_3x1, qRH_4x1, RElb_ang);
    kine_drc.FK_LeftHand_Local(Q_filt_34x1, LOCAL_PELV, pLH_3x1, qLH_4x1, LElb_ang);
    kine_drc.FK_COM_Local(Q_filt_34x1, pCOM_2x1);
    pPelZ = Q_filt_34x1[idZ];
    for(int i=0; i<3; i++){
        wbPosRF[i]->SetRetValue(pRF_3x1[i]);
...
void TaskMotion::WBIK_UB_PELV_LH(){
    int RH_ref_frame = LOCAL_UB;
    int LH_ref_frame = LOCAL_PELV;
    kine_drc.IK_UpperBody_Local(des_pRH_3x1, des_qRH_4x1, des_RElb_ang, RH_ref_frame, des_pLH_3x1, des_qLH_4x1, des_LElb_ang, LH_ref_frame, des_rWST, Qout_34x1);
