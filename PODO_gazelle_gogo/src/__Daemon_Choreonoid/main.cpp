...
int RBCore_SMInitialize(){
    ...
    if(shmFD == -1){
        FILE_LOG(logERROR) << "Fail to open SIMULSYNC shared memory";
        return -1;
    }else{
        if(ftruncate(shmFD, sizeof(int)) == -1){
            FILE_LOG(logERROR) << "Fail to truncate SIMULSYNC shared memory";
            return -1;
        }else{
            GAZEBO_FLAG = (int *)mmap(0, sizeof(int), PROT_READ|PROT_WRITE, MAP_SHARED, shmFD, 0);
            if(sharedCMD == (void*)-1){
                FILE_LOG(logERROR) << "Fail to mapping SIMULSYNC shared memory";
                return -1;
            }
        }
    }
    FILE_LOG(logSUCCESS) << "SIMULSYNC shared memory creation = OK [SYMULSYNC]";
    // =========================================================================
