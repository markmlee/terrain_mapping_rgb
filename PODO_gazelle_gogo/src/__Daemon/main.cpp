...
double theta_old = 0;
double psi_old = 0;
bool IMU_reset_flag = false;

// Thread Functions
int     NEW_FOG_CAN_CHANNEL = 2;
int     NEW_FOG_CAN_RX_ID_1 = 86;
int     NEW_FOG_CAN_RX_ID_2 = 87;
int     NEW_FOG_CAN_TX_ID = 151;
void    THREAD_ReadNewFOG(void);
...
void    RBCMD_SensorIMU_Zero(); //Hyobin added
...
//            }
        ...
        case DAEMON_SENSOR_IMU_ZERO: //Hyobin added
            FILE_LOG(logINFO) << "CMD: DAEMON_SENSOR_IMU_ZERO";
            if(__IS_GAZEBO) {FILE_LOG(logINFO) << "Gazebo doesn't need DAEMON_SENSOR_IMU_NULL";}
            else{
                if(IS_CAN_OK)   {RBCMD_SensorIMU_Zero();}
                else            {FILE_LOG(logWARNING) << "CAN device not set";}
            }
            sharedCMD->COMMAND[RBCORE_PODO_NO].USER_COMMAND = NO_ACT;
            break;
        ...
        case 321:
            {
            char ctype = sharedCMD->COMMAND[RBCORE_PODO_NO].USER_PARA_CHAR[0];

            if(ctype == 1){
                FILE_LOG(logINFO) << "New FOG Data Enable";
                RBCAN_MB mb;
                mb.channel = NEW_FOG_CAN_CHANNEL;
                mb.id = NEW_FOG_CAN_TX_ID;
                mb.data[0] = 1;
                mb.data[1] = 1;
                mb.dlc = 2;
                canHandler->RBCAN_WriteData(mb);
            }else if(ctype == 2){
                FILE_LOG(logINFO) << "New FOG Nulling";
                RBCAN_MB mb;
                mb.channel = NEW_FOG_CAN_CHANNEL;
                mb.id = NEW_FOG_CAN_TX_ID;
                mb.data[0] = 3;
                ...
                FILE_LOG(logINFO) << "New FOG Reset";

                int cur_roll = (int)(sharedSEN->IMU[0].Roll_Comp * 100.);
                int cur_pitch = (int)(sharedSEN->IMU[0].Pitch_Comp * 100.);
                int cur_yaw = (int)(0. * 100.);

                std::cout<<"FOG will be reset as : "<<cur_roll/100.<<" and "<<cur_pitch/100.<<std::endl;
                ...
                mb.data[2] = (unsigned char)(cur_roll & 0x00FF);
                mb.data[3] = (unsigned char)((cur_roll>>8) & 0x00FF);
                mb.data[4] = (unsigned char)(cur_pitch & 0x00FF);
                mb.data[5] = (unsigned char)((cur_pitch>>8) & 0x00FF);
                mb.data[6] = (unsigned char)(cur_yaw & 0x00FF);
                mb.data[7] = (unsigned char)((cur_yaw>>8) & 0x00FF);
                mb.dlc = 8;
                canHandler->RBCAN_WriteData(mb);
            }else if(ctype == 4){
                FILE_LOG(logINFO) << "New FOG Zero";

                std::cout<<"FOG will be reset as : "<<0<<" and "<<0<<std::endl;
                ...
                FILE_LOG(logINFO) << "New FOG Earth Compen-mode Reset";
                ...
                FILE_LOG(logINFO) << "New FOG Earth Compen-mode Zero";
...
void RBCore_RTThreadCon(void *)
{
    ...
    while(IS_WORKING)
    {
        ...
        for(int i=0; i<_NO_OF_MC; i++){
            ...
            //gazelle
//            //right leg
//            int MO_RAP = sharedCMD->MotionOwner[4][0];
//            int MO_RAR = sharedCMD->MotionOwner[5][0];
//            double RA1_deg, RA2_deg;
//            IK_Ankle(sharedREF->JointReference[MO_RAP][4][0], sharedREF->JointReference[MO_RAR][5][0], RA1_deg, RA2_deg);
//            _DEV_MC[4].Joints[0].Reference = RA1_deg;
//            _DEV_MC[5].Joints[0].Reference = RA2_deg;
//            sharedSEN->ENCODER[4][0].CurrentReference = RA1_deg;
//            sharedSEN->ENCODER[5][0].CurrentReference = RA2_deg;

//            //left leg
//            int MO_LAP = sharedCMD->MotionOwner[10][0];
//            int MO_LAR = sharedCMD->MotionOwner[11][0];
//            double LA1_deg, LA2_deg;
//            IK_Ankle(sharedREF->JointReference[MO_LAP][10][0], sharedREF->JointReference[MO_LAR][11][0], LA1_deg, LA2_deg);
//            _DEV_MC[10].Joints[0].Reference = LA1_deg;
//            _DEV_MC[11].Joints[0].Reference = LA2_deg;
//            sharedSEN->ENCODER[10][0].CurrentReference = LA1_deg;
//            sharedSEN->ENCODER[11][0].CurrentReference = LA2_deg;

//            cout<<RA1_deg<<", "<<RA2_deg<<", "<<LA1_deg<<", "<<LA2_deg<<endl;
...
void RBCMD_SensorSensorOnOff(){
    ...
    _DEV_IMU[0].RBIMU_RequestData(0xbc);
...
void RBCMD_SensorIMU_Zero(){
    ...
    if(id == -1){   //All
        for(int i=0; i<_NO_OF_IMU; i++){
            ...
            _DEV_IMU[i].FOGQ0 = 0.5*sqrt(1.0 + cos(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) + sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R) + cos(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].YAW*D2R) + cos(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].PITCH*D2R) );

            _DEV_IMU[i].FOGQ1 = (sin(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].PITCH*D2R) - (cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R) - sin(_DEV_IMU[i].ROLL*D2R)*cos(_DEV_IMU[i].YAW*D2R)))/4.0;

            _DEV_IMU[i].FOGQ2 = (cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) + sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].YAW*D2R) - (-sin(_DEV_IMU[i].PITCH*D2R)))/4.0;

            _DEV_IMU[i].FOGQ3 = ((cos(_DEV_IMU[i].PITCH*D2R)*sin(_DEV_IMU[i].YAW*D2R)) - (sin(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].PITCH*D2R)*cos(_DEV_IMU[i].YAW*D2R) - cos(_DEV_IMU[i].ROLL*D2R)*sin(_DEV_IMU[i].YAW*D2R)))/4.0;
        ...
        _DEV_IMU[id].FOGQ0 = 0.5*sqrt(1.0 + cos(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) + sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R) + cos(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].YAW*D2R) + cos(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].PITCH*D2R) );

        _DEV_IMU[id].FOGQ1 = (sin(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].PITCH*D2R) - (cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R) - sin(_DEV_IMU[id].ROLL*D2R)*cos(_DEV_IMU[id].YAW*D2R)))/4.0;

        _DEV_IMU[id].FOGQ2 = (cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) + sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].YAW*D2R) - (-sin(_DEV_IMU[id].PITCH*D2R)))/4.0;

        _DEV_IMU[id].FOGQ3 = ((cos(_DEV_IMU[id].PITCH*D2R)*sin(_DEV_IMU[id].YAW*D2R)) - (sin(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].PITCH*D2R)*cos(_DEV_IMU[id].YAW*D2R) - cos(_DEV_IMU[id].ROLL*D2R)*sin(_DEV_IMU[id].YAW*D2R)))/4.0;
...
int RBCore_CANInitialize(){
    ...
    if(canHandler->IsWorking() == false){
        ...
        canHandler->RBCAN_AddMailBox(86);
        canHandler->RBCAN_AddMailBox(87);
...
void    THREAD_ReadNewFOG(void){
    ...
    if(mb.status != RBCAN_NODATA){
        ...
        sharedSEN->FOG.Roll = phi;
        sharedSEN->FOG.Pitch = theta;
        sharedSEN->FOG.Yaw = psi;


        //std::cout<<"Hello : "<<phi<<std::endl;

        mb.status = RBCAN_NODATA;
    }

    mb2.channel = NEW_FOG_CAN_CHANNEL;
    mb2.id = NEW_FOG_CAN_RX_ID_2;
    ...
    if(mb2.status != RBCAN_NODATA){
        ...
        int check_sum = mb2.data[6];
//        std::cout<<"FOG Check: "<<check_sum<<std::endl;

        sharedSEN->FOG.RollVel = roll_vel;
        sharedSEN->FOG.PitchVel = pitch_vel;
        sharedSEN->FOG.YawVel = yaw_vel;
...
void THREAD_ReadNewIMU(void){
    ...
    if(mb.status != RBCAN_NODATA){
        ...
        // roll pitch High pass filtering
...
//        double PI = 3.141592;
//        double dt = 0.002;
//        double alpha_IMU_HPF = 1.0/(1.0 + 2.0*PI*dt*0.0001);
//        sharedSEN->IMU[0].Pitch = alpha_IMU_HPF*(sharedSEN->IMU[0].Pitch + (theta - theta_old));
//        sharedSEN->IMU[0].Roll = alpha_IMU_HPF*(sharedSEN->IMU[0].Roll + (phi - phi_old));
//        theta_old = theta;
//        phi_old = phi;


        sharedSEN->IMU[0].Yaw = psi;
        sharedSEN->IMU[0].Pitch = theta + _DEV_IMU[0].PITCH_OFFSET;
        sharedSEN->IMU[0].Roll = phi + _DEV_IMU[0].ROLL_OFFSET;
...
//        sharedSEN->IMU[i].Q[0]      = _DEV_IMU[i].FOGQ0;
//        sharedSEN->IMU[i].Q[1]      = _DEV_IMU[i].FOGQ1;
//        sharedSEN->IMU[i].Q[2]      = _DEV_IMU[i].FOGQ2;
//        sharedSEN->IMU[i].Q[3]      = _DEV_IMU[i].FOGQ3;
//    }
...
//    FK_diff_Ankle(_DEV_MC[4].Joints[0].CurrentPosition, _DEV_MC[5].Joints[0].CurrentPosition, 0,0,RAP_deg, RAR_deg);
//    sharedSEN->ENCODER[4][0].CurrentPosition = RAP_deg;
//    sharedSEN->ENCODER[5][0].CurrentPosition = RAR_deg;

//    double LAP_deg, LAR_deg;
//    FK_diff_Ankle(_DEV_MC[10].Joints[0].CurrentPosition, _DEV_MC[11].Joints[0].CurrentPosition, 0,0,LAP_deg, LAR_deg);
//    sharedSEN->ENCODER[10][0].CurrentPosition = LAP_deg;
//    sharedSEN->ENCODER[11][0].CurrentPosition = LAR_deg;
