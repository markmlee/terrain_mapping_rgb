...
typedef struct _CHOREONOID_
{
    ...
    bool            AnkleTorqueCon_flag;
    double          T_RAR, T_RAP, T_LAR, T_LAP;
} CHOREONOID, *pCHOREONOID;

typedef struct _STATE_ESTIMATE_
{
    float	    X;
    float	    dX;
    float	    Y;
...
} STATE_ESTIMATE, *pSTATE_ESTIMATE;
...
typedef enum _COMMAND_SET_{
    ...
    //
    POWER_CONTROL,
    DAEMON_SENSOR_IMU_ZERO, //Hyobin added
} COMMAND_SET;
